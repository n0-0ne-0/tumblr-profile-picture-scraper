...
- You can let it run for as long as you want; it will keep fetching until you're satisfied (or until your API keys run out).
...
For real now, i originally made one that scraped the page with Playwright, but it's much cleaner to just use the public API.

## API keys

Add your API keys either as environment variables or in a `.env` file following the [example](.env.example) layout.

After creating an account on Tumblr and verifying your email, it's easy to create multiple API keys at https://www.tumblr.com/oauth/apps (most fields are optional or accept anything you put in).
...
After that you just watch it work until you are satisfied with the number of pictures. You could leave it running until you have all the pictures from all the likes from all the posts, but normally that's overkill.
